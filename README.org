...
Then, to make sure that the right init-lang.el file is loaded when a
respective file is opened, we use the following elisp snippet:
...
Most people would agree that this setup is somewhat redundant. For
example, the path "lisp/core/core-appearance" has core listed
twice. So we might try the following instead:
...
The reason that I decided to not choose the flattened directory layout
was due to the additionial core-*.el files that
~loaddefs-generate-batch~ would scan over. As many enthusiastic Emacs
users might be able to tell, this Emacs config was heavily based on
aboabo's Emacs config. When looking at his configuration, I was
curious about how many files in his modes directory did NOT contain
autoloads. I used the grep command to determine this:
...
Admittedly, I do not follow this function as much as I would like, but
I will try to prefix personal functions with "my/" in the future.
...
I just use plain old require along with Borg to manage packages.
...
of your Emacs config into one single Org Mode file or a single init.el?

Admittedly, some of the debugging benefits due to a multiple file
setup are lost since Emacs frustratingly does not tell what line and
what file an error occurs in.
